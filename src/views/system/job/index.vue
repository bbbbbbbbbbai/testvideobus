  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="jobList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="是否并发" align="center">
        <template slot-scope="scope">
          <el-tag type="success" v-if="scope.row.concurrent">是</el-tag>
          ...
          <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="handleRun" icon="el-icon-caret-right"
                v-hasPermi="['system:job:execute']">执行一次</el-dropdown-item>
            ...
            <el-form-item prop="invokeTarget">
              <span slot="label">
                ...
                <el-tooltip placement="top">
                  <div slot="content">
                    Bean调用示例：aaTask.bbParams('cc')
                    <br />Class类调用示例：com.azor.quartz.task.AaTask.bbParams('cc')
    ...
    <el-dialog title="任务详细" :visible.sync="openView" width="700px" append-to-body>
      <el-form ref="form" :model="form" label-width="120px" size="mini">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="任务分组：">{{ selectDictLabel(this.dict.type.sys_job_group, form.jobGroup) }}
            </el-form-item>
            <el-form-item label="创建时间：">
              {{ form.updateInfo != null ? parseTime(form.updateInfo.createdTime) : '' }}
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
<script>
import { listJob, getJob, delJob, addJob, updateJob, runJob, changeJobStatus } from "@/api/system/job";
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.jobName + '"任务吗？').then(function() {
        ...
        row.status = row.status === "PAUSE" ? "NORMAL" : "PAUSE";
      });
    },
    /* 立即执行一次 */
    handleRun(row) {
    ...
    handleJobLog(row) {
      ...
      this.$router.push({ path: '/system/job-log/index', query: { jobId: jobId } })
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.reset();
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm(jobIds instanceof Array ? '确定删除这些定时任务？' : '是否确认删除定时任务"' + row.jobName + '"？').then(function() {
