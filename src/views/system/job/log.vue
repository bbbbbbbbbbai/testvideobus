    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:jobLog:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="jobLogList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="任务组名" align="center" prop="jobGroup" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <span v-for="item in jobGroups" :key="item.id" v-if="item.value==scope.row.jobGroup">{{item.label}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="执行状态" align="center" prop="status">
        <template slot-scope="scope">
          ...
          <el-tag type="danger" v-if="scope.row.status == 'FAIL'">失败</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="结束执行时间" align="center" prop="startTime" width="180">
        <template slot-scope="scope">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="结束执行时间：">
              {{ form.stopTime != null ? parseTime(form.stopTime) : '' }}
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
export default {
  ...
  created() {
    ...
    this.getDicts('sys_job_group');
    if (jobId !== undefined && jobId != 0) {
      this.jobId = jobId;
      getJob(jobId).then(response => {
        this.queryParams.jobId = response.data.id;
