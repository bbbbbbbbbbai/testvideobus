  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tableList" @selection-change="handleSelectionChange">
      ...
      <el-table-column
        ...
        width="120"
      />
      <el-table-column label="创建时间" prop="updateInfo.createdTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.updateInfo.createdTime) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="更新时间" prop="updateInfo.updateTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.updateInfo.updateTime) }}</span>
        </template>
      </el-table-column>
...
<script>
  import {listTable, previewTable, delTables, genCode, syncTable} from "@/api/tool/gen";
    ...
    methods: {
      ...
      handleGenTable(row) {
        ...
        if (row.genType === "1") {
          ...
          this.$download.zip("/tool/gen/batchGenCode?tableNames=" + tableNames, "generator");
        }
      },
      /** 同步数据库操作 */
      handleSyncTable(row) {
